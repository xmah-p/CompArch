...
- 流程：
  ...
  - （对 DRAM 芯片预充电）
  - ACT（ACTivate）或 RAS（Row Access Strobe）：内存控制器将 $i$ 广播给其所有 DRAM 芯片。DRAM 芯片的行译码器将 $i$ 译码，选中对应的行，将行数据经放大放在行缓冲区中
  - 等待 tRCD（Row to Column Delay）长的时间
  - CAS：内存控制器将 $j$ 广播给其所有 DRAM 芯片。DRAM 芯片从行缓冲区中取出 $j$ 列的数据并输出
  - 等待 CL（CAS Latency）长的时间
  - 内存控制器采样数据，经过系统总线送入 CPU
- CAS 可以读多个数，每个时钟周期都能输出一个数 
- 如果下一次访问的数据在同一行，则可以省略 RAS 和 tRCD
- 否则预充电，将这一行关闭
- 预充电既可以在选中新行时进行，也可以在一次传输只剩最后一个字时进行（可以和最后一个字的传输重叠）
- 性能参数：
  - tRCD（Row to Column Delay）：从 RAS 到 CAS 的时间，约 2~3 个时钟周期（15~23ns）
  - CL（CAS Latency）：从 CAS 到数据输出的时间，约 2~3 个时钟周期
  - tRP（Row Precharge）：从预充电到 RAS 的时间，约 2~3 个时钟周期
- 两次行地址不同的 SDRAM 读需要间隔 tRCD + CL + 数据个数 - 1 + tRP 个时钟周期
...
DDR2-400 核心频率仍为 100MHz，但每次从核心取出 4 个数据，I/O 频率提升到了 200MHz，等效频率为 400MHz。
